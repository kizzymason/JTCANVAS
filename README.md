...
- 账号与计费：用户名密码登录、人民币余额钱包、卡密兑换、图片按张与视频按秒计费、生成前显示预估价格、失败自动全额退回。
- 管理后台：左侧边栏 SaaS 布局，管理用户、渠道与模型、价格与计费方式、卡密、财务流水与对账、存储策略、生成记录与审计日志。
- AI 创作：由服务端代理你在后台配置的 OpenAI 兼容 / Gemini / PiAPI 接口，支持文生图、图生图、参考图编辑、文本问答、音频和视频生成。
- 画布助手：围绕选中节点和上游节点对话、生图，并把结果插回画布。
- 本地 Agent：通过本机 Canvas Agent 连接 Codex / Claude Code，让 Agent 通过 MCP 操作当前画布；
- Codex App 插件：提供 Codex app 插件，安装后会自动注册 MCP 并尝试拉起本地 Agent。
- 插件系统：支持通过 URL 动态安装 / 启用 / 更新 / 卸载远程节点插件，并提供 TypeScript SDK 自行开发画布节点插件。
- 自定义接口调用：管理员可为每个模型编写自定义调用脚本，在服务端沙箱中执行，灵活适配各类中转站与自建服务。
- 存储可选：生成结果与上传文件可存本机磁盘或 S3 兼容对象存储，在管理后台切换。
...
项目由前端（`web/`）与后端（`server/`）两部分组成。账号、余额、画布、素材和生成结果都保存在服务端；第三方 AI API Key 只存在服务端并加密入库，由管理员在管理后台配置，浏览器不持有任何密钥。
...
运行后默认端口 3000，可访问 `http://localhost:3000`。会启动 nginx（前端 + `/api` 反代）、api、worker、postgres、redis 共五个服务。

**第一个注册的账号会自动成为管理员。** 注册后进入右上角管理后台，先添加渠道与模型、为模型设置价格，用户才能开始生成。
...
cd ../web
bun install
bun run dev            # 默认 3000，通过 /api 访问后端
```
