...
- CSS Modules 中匹配 `html.gravity`、`.dark` 等全局主题根节点时必须用 `:global(...)`，并在实际深浅主题 DOM 中核对计算样式，避免选择器被模块化后失效。
...
- 前台深浅色使用独立的 `useFrontendThemeStore` 偏好，默认暗色并由顶栏切换；暗色保持黑底荧光绿，亮色使用纯白表面与曜石黑强调。前台不得读取或修改管理后台的 `useThemeStore` 主题偏好。
...
- 第三方 API Key、存储凭据与支付商户密钥必须 AES-256-GCM 加密后入库，明文只在内存中短暂存在；任何接口（包括管理接口）都只回传掩码，不回传明文。支付渠道 PID/密钥不得写入 Git，仅通过环境变量 `ZPAY_PID`/`ZPAY_KEY` 在渠道表为空时预置，或由管理员在「支付渠道」页填写。
...
- 前台画布通过 `useFrontendThemeStore` 实时选择 `canvasThemes`；仅管理后台界面使用管理后台主题状态。
- 前台画布通过 `useFrontendThemeStore` 实时选择 `canvasThemes`；仅管理后台界面使用管理后台主题状态。
...
- 访客统计与注册防刷不得阻塞首屏：页面载入时不要做 canvas 指纹，也不要同步等待上报入库。beacon 在 `load` 后空闲发送，接口先回 200 再异步写库，且不要为上报去 Redis 查会话。注册弹窗不要滑块验证；防刷只保留本机设备 ID 与 IP 限制，指纹失败不得把所有访客收成同一个哈希。
