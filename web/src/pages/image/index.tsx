...
export default function ImagePage() {
    ...
    const imageModel = useModelStore((state) => state.find(model) ?? state.models.find((item) => item.modelName === model));
    const canGenerate = Boolean(prompt.trim());
    const generationCount = Math.max(1, Math.min(modelFeaturesOf(imageModel).maxCount, Number(config.count) || 1));
    // Blocks the request client-side when the wallet cannot cover it; the server rejects it too.
    const { affordable } = useCanAffordGeneration(model, { count: generationCount, spec: pricingSpecFor(effectiveConfig.quality, effectiveConfig.size), referenceCount: references.length });
    ...
    const deleteSelectedLogs = () => {
        // Only the local workbench log is removed. The authoritative record lives on the server under
        // the account's usage history, and result files are reclaimed there by reference counting.
                ...
                <section className="grid min-w-0 gap-3 lg:min-h-0 lg:overflow-hidden xl:grid-cols-[420px_minmax(0,1fr)]">
                    <div className="thin-scrollbar flex min-w-0 flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-6 min-w-0 space-y-5">
                            ...
                            <div className="min-w-0">
                                <div className="mb-2 flex min-w-0 items-center justify-between gap-3">
                                    <span className="min-w-0 truncate text-base font-semibold">{t("imageWorkbench.references")}</span>
                                    <div className="flex shrink-0 gap-2">
                            ...
                            <div className="flex min-w-0 max-w-full items-center gap-2 overflow-hidden rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="min-w-0 flex-1 truncate text-stone-500 dark:text-stone-400" title={`${modelOptionLabel(model)} · ${effectiveConfig.size} · ${effectiveConfig.quality}`}>
                                    {modelOptionLabel(model)} · {effectiveConfig.size} · {effectiveConfig.quality}
                                </span>
                        ...
                        <div className="mt-auto pt-6">
                            <div className="mb-2 flex justify-end">
                                <PriceEstimate model={model} count={Number(effectiveConfig.count) || 1} spec={pricingSpecFor(effectiveConfig.quality, effectiveConfig.size)} referenceCount={references.length} />
                            </div>
    ...
    return (
        <>
            ...
            <div className="col-span-2">
                <ImageSettingsPanel config={config} onConfigChange={(key, value) => updateConfig(key, value)} theme={theme} showTitle={false} className="space-y-4" modelValue={model} />
            </div>
        </>
    );
}
