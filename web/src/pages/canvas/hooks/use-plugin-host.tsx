    ...
    const pluginAi = useMemo<CanvasPluginAi>(() => {
        ...
        return {
            generateImage: async (prompt, options) => {
                if (!services.imageEnabled) throw new Error(t("canvas.serviceDisabled.image"));
            ...
            generateVideo: async (prompt, options) => {
                if (!services.videoEnabled) throw new Error(t("canvas.serviceDisabled.video"));
                const config = {
            ...
            listModels: (capability) => selectableModelsByCapability(effectiveConfig, (capability as ModelCapability) || "image").map((value) => ({ value, label: decodeChannelModel(value)?.model || value })),
            defaultModel: (capability) => buildGenerationConfig(effectiveConfig, undefined, capability).model,
        };
    }, [effectiveConfig, isAiConfigReady, openConfigDialog, services.imageEnabled, services.videoEnabled, t]);
