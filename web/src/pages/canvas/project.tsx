...
function InfiniteCanvasPage() {
    ...
    const [nodeCreateMenu, setNodeCreateMenu] = useState<{ world: Position; screen: Position } | null>(null);
    ...
    // Files are reclaimed by server-side reference counting once nothing points at them, so the
    // client no longer sweeps storage. Kept as a no-op so the call sites stay untouched.
    const cleanupCanvasFiles = useCallback((_extra?: unknown) => {}, []);
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            // Full canvas data now comes from the server, so opening a project is asynchronous.
    ...
    }, [fragmentBootstrap, localAgentEnabled, openAgentPanel, projectLoaded, searchParams, services.agentEnabled]);
    ...
    const createConnectedNode = useCallback(
        (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Config | CanvasNodeType.Video | CanvasNodeType.Audio, pending: PendingConnectionCreate) => {
            const metadata = type === CanvasNodeType.Config ? { model: effectiveConfig.imageModel || effectiveConfig.model, size: effectiveConfig.size, count: getImageGenerationCount(effectiveConfig.canvasImageCount || effectiveConfig.count, effectiveConfig.imageModel || effectiveConfig.model) } : undefined;
    ...
    const visibleNodes = useMemo(() => {
        ...
        return nodes.filter((node) => isCanvasNodeServiceEnabled(node.type, services) && node.position.x + node.width > viewLeft && node.position.x < viewRight && node.position.y + node.height > viewTop && node.position.y < viewBottom);
    }, [nodes, services, size.height, size.width, viewport.k, viewport.x, viewport.y]);

    const listedNodes = useMemo(() => nodes.filter((node) => isCanvasNodeServiceEnabled(node.type, services)), [nodes, services]);
    ...
    const createNode = useCallback(
        (type: CanvasNodeTypeId, position?: Position) => {
            if (!isCanvasNodeServiceEnabled(type, services)) return;
            ...
            const configMetadata =
                type === CanvasNodeType.Config
                    ? {
                          ...
                          count: getImageGenerationCount(effectiveConfig.canvasImageCount || effectiveConfig.count, effectiveConfig.imageModel || effectiveConfig.model),
                      }
                    : undefined;
        ...
        [effectiveConfig.canvasImageCount, effectiveConfig.count, effectiveConfig.imageModel, effectiveConfig.model, effectiveConfig.size, getCanvasCenter, services],
    );
    ...
    const handleGlobalPointerMove = useCallback(
        (event: PointerEvent) => {
            ...
            nodesRef.current
                .forEach((node) => {
                    if (!isCanvasNodeServiceEnabled(node.type, services)) return;
    ...
    const createImageFileNode = useCallback(async (file: File, position: Position) => {
        if (!services.imageEnabled) return;
        ...
        setSelectedConnectionId(null);
        setDialogNodeId(id);
    }, [services.imageEnabled]);

    const createVideoFileNode = useCallback(async (file: File, position: Position) => {
        if (!services.videoEnabled) return;
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (isModifierShortcut && !event.altKey && key === "a") {
                event.preventDefault();
                setSelectedNodeIds(new Set(nodesRef.current.filter((node) => isCanvasNodeServiceEnabled(node.type, services)).map((node) => node.id)));
                setSelectedConnectionId(null);
                setContextMenu(null);
                setSelectionBox(null);
                return;
    ...
    }, [copySelectedNodes, deleteConnection, deleteNodes, pasteCopiedNodes, pasteSystemClipboard, redoCanvas, selectedConnectionId, services, setConnecting, undoCanvas]);
    ...
    const captureVideoNodeFrame = useCallback(
        async (nodeId: string, position: VideoFramePosition) => {
            setContextMenu(null);
            if (!services.imageEnabled) return message.error(t("canvas.serviceDisabled.image"));
            const node = nodesRef.current.find((item) => item.id === nodeId);
        ...
        [message, services.imageEnabled, t],
    );
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            try {
                ...
                message.error(error instanceof ApiError ? error.message : t("assets.saveFailed"));
            }
        },
        [addAsset, message, t],
    );
    ...
    const handleImageInputChange = useCallback(
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            const files = Array.from(event.target.files || []).filter(
                (f) => (f.type.startsWith("image/") && services.imageEnabled) || (f.type.startsWith("video/") && services.videoEnabled) || isAudioFile(f),
            );
            if (!files.length) {
        ...
        [createAudioFileNode, createImageFileNode, createVideoFileNode, screenToCanvas, services.imageEnabled, services.videoEnabled, size.height, size.width],
    );
    ...
    const handleDrop = useCallback(
        (event: ReactDragEvent<HTMLDivElement>) => {
            ...
            const files = Array.from(event.dataTransfer.files).filter(
                (item) => (item.type.startsWith("image/") && services.imageEnabled) || (item.type.startsWith("video/") && services.videoEnabled) || isAudioFile(item),
            );
        ...
        [createAudioFileNode, createImageFileNode, createVideoFileNode, screenToCanvas, services.imageEnabled, services.videoEnabled],
    );
    ...
    const handleCanvasContextMenu = useCallback(
        (event: ReactMouseEvent) => {
            if ((event.target as HTMLElement).closest("[data-node-id],[data-connection-id],[data-canvas-no-zoom],[data-connection-create-menu]")) return;
            event.preventDefault();
            setContextMenu(null);
            if (referencePickerNodeId) return;
            setNodeCreateMenu({
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            if (mode === "image" && !services.imageEnabled) {
                message.error(t("canvas.serviceDisabled.image"));
                return;
            }
            if (mode === "video" && !services.videoEnabled) {
                message.error(t("canvas.serviceDisabled.video"));
                return;
            }
            ...
            try {
                if (mode === "image") {
                    const count = getImageGenerationCount(generationConfig.count, generationConfig.model);
                ...
                if (mode === "video") {
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, [...generationContext.referenceImages, ...generationContext.referenceVideos], { signal: controller.signal }),
                        );
        ...
        [effectiveConfig, finishGenerationRequest, isAiConfigReady, message, openConfigDialog, services.imageEnabled, services.videoEnabled, startGenerationRequest, t],
    );
    useEffect(() => {
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            if (node.type === CanvasNodeType.Image && !services.imageEnabled) {
                message.error(t("canvas.serviceDisabled.image"));
                return;
            }
            if (node.type === CanvasNodeType.Video && !services.videoEnabled) {
                message.error(t("canvas.serviceDisabled.video"));
                return;
            }
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, prompt, [...retryImages, ...(context?.referenceVideos ?? [])], { signal: controller.signal }));
    ...
    const generateImageFromTextNode = useCallback(
        ...
        [effectiveConfig.canvasImageCount, effectiveConfig.count, effectiveConfig.imageModel, effectiveConfig.model, effectiveConfig.size, message, services.imageEnabled, t],
    );
    ...
    const insertAssistantImage = useCallback(
        ...
        [screenToCanvas, services.imageEnabled, size.height, size.width],
    );
    ...
    const handleAssetInsert = useCallback(
        (payload: InsertAssetPayload) => {
            if (payload.kind === "text") {
                ...
                if (!services.videoEnabled) {
                    message.warning(t("canvas.serviceDisabled.video"));
                    return;
                }
                ...
                if (!services.imageEnabled) {
                    message.warning(t("canvas.serviceDisabled.image"));
                    return;
                }
        ...
        [insertAssistantImage, insertAssistantText, message, screenToCanvas, services.imageEnabled, services.videoEnabled, size.height, size.width, t],
    );
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <CanvasSidePanel nodes={listedNodes} selectedNodeIds={selectedNodeIds} onFocusNode={focusNode} onPreviewNode={setPreviewNodeId} onInsertAsset={handleAssetInsert} />
            <section className="relative min-w-0 flex-1 overflow-hidden">
                    ...
                    <svg className="absolute left-0 top-0 h-[10000px] w-[10000px] overflow-visible" style={{ pointerEvents: "none", transform: "translateZ(0)", zIndex: 0 }}>
                        {connections
                            .map((connection) => {
                                ...
                                if (!isCanvasNodeServiceEnabled(from.type, services) || !isCanvasNodeServiceEnabled(to.type, services)) return null;
                ...
                {isMiniMapOpen ? <Minimap nodes={listedNodes} viewport={viewport} viewportSize={size} onViewportChange={setViewport} /> : null}

                <CanvasZoomControls scale={viewport.k} onScaleChange={setZoomScale} onReset={resetViewport} isMiniMapOpen={isMiniMapOpen} onToggleMiniMap={() => setIsMiniMapOpen((value) => !value)} />

                {nodeCreateMenu ? (
                    <NodeCreateMenu
                        screen={nodeCreateMenu.screen}
                        onCreate={(type) => {
                            createNode(type, nodeCreateMenu.world);
                            setNodeCreateMenu(null);
                        }}
