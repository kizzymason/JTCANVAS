...
import { siteServicesFrom } from "@/hooks/use-site-services";
...
function assertWorkbenchService(kind: "image" | "video") {
    const services = siteServicesFrom(useAuthStore.getState().site);
    if (kind === "image" && !services.imageEnabled) throw new Error(siteText("imageServiceClosed"));
    if (kind === "video" && !services.videoEnabled) throw new Error(siteText("videoServiceClosed"));
}
...
function getImageConfig() {
    ...
    const publicModel = useModelStore.getState().models.find((item) => item.value === model);
    const features = modelFeaturesOf(publicModel);
    return {
...
function runImageWorkbench(input: SiteToolInput, navigate: NavigateFunction) {
    ...
    if (typeof input.model === "string" && input.model.trim()) {
        const value = normalizeModelOptionValue(input.model, useModelStore.getState().models) || input.model;
    ...
    if (input.count != null) {
        const maxCount = modelFeaturesOf(useModelStore.getState().models.find((item) => item.value === (useConfigStore.getState().config.imageModel || useConfigStore.getState().config.model))).maxCount;
        const count = String(Math.max(1, Math.min(maxCount, Math.floor(Number(input.count)) || 1)));
...
function getVideoConfig() {
    ...
    const features = modelFeaturesOf(useModelStore.getState().models.find((item) => item.value === model));
    return {
        current: {
            model,
