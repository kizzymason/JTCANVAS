...
export function getGenerationCount(count: string, maxCount = 15) {
    return Math.max(1, Math.min(maxCount, Math.floor(Math.abs(Number(count)) || 1)));
}

export function getImageGenerationCount(count: string, modelValue?: string) {
    const models = useModelStore.getState().models;
    const model = models.find((item) => item.value === modelValue) ?? models.find((item) => Boolean(modelValue) && item.modelName === modelValue);
    return getGenerationCount(count, modelFeaturesOf(model).maxCount);
}
