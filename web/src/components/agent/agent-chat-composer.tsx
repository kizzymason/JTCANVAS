...
import { ArrowUp, Check, ChevronUp, Gauge, Hand, ImagePlus, LoaderCircle, RefreshCw, ShieldAlert, ShieldCheck, ShieldOff, Square, X } from "lucide-react";
                            ...
                            <div key={item.id} className="group relative size-14 shrink-0 overflow-hidden rounded-xl border" style={{ borderColor: theme.node.stroke }} title={item.name}>
                                ...
                                {onRemoveAttachment ? (
                                    <button
                                        ...
                                        aria-label={t("agent.composer.removeImage")}
                                    >
                                        <X className="size-3" />
                                    </button>
                                ) : null}
                            ...
                            <>
                                ...
                                <Tooltip title={t("agent.composer.uploadImage")}>
                                    <Button
                                        ...
                                        aria-label={t("agent.composer.uploadImage")}
                                    />
                                </Tooltip>
                            </>
                        ) : null}
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                        {sending && onStop ? (
                            <Tooltip title={t("agent.composer.stop")} placement="top">
                                <Button danger shape="circle" className="!h-10 !w-10 !min-w-10" icon={<Square className="size-4" />} onClick={() => void onStop()} aria-label={t("agent.composer.stop")} />
                            </Tooltip>
                        ) : (
                ...
                <span className="inline-flex shrink-0">
                    <Select value={model} open={modelOpen} onOpenChange={setModelOpen} onValueChange={onModelChange}>
                        <SelectTrigger
                            ...
                            aria-label={t("agent.composer.selectModel", { model: current.displayName || current.model })}
                        >
                            <ModelBrandIcon model={current.model} className="size-3.5" />
