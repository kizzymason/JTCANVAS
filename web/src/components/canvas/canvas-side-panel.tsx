...
function CanvasNodesTab({ nodes, selectedNodeIds, onFocusNode, onPreviewNode, theme }: { nodes: CanvasNodeData[]; selectedNodeIds: Set<string>; onFocusNode: (nodeId: string) => void; onPreviewNode: (nodeId: string) => void; theme: CanvasTheme }) {
    ...
    const filterValues = NODE_FILTER_VALUES.filter((value) => value === "all" || isCanvasNodeServiceEnabled(value, services));

    const filtered = useMemo(() => {
        const query = keyword.trim().toLowerCase();
        return nodes.filter((node) => isCanvasNodeServiceEnabled(node.type, services) && (typeFilter === "all" || node.type === typeFilter) && (!query || [node.title, node.metadata?.content, node.metadata?.prompt].filter(Boolean).join(" ").toLowerCase().includes(query)));
    }, [nodes, keyword, typeFilter, services]);
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="flex items-center gap-2 px-3 pb-2.5 pt-1">
                ...
                {selectMode ? null : <Select size="small" variant="borderless" className="w-20" value={filterValues.includes(typeFilter) ? typeFilter : "all"} onChange={setTypeFilter} options={filterValues.map((value) => ({ value, label: value === "all" ? t("common.all") : t(`canvas.sidePanel.filter.${value}`) }))} />}
            </div>
...
const CanvasAssetsTab = memo(function CanvasAssetsTab({ onInsert, theme }: { onInsert: (payload: InsertAssetPayload) => void; theme: CanvasTheme }) {
    ...
    const groups = useMemo(
        () =>
            ASSET_GROUPS.filter((group) => (group.kind === "image" ? services.imageEnabled : group.kind === "video" ? services.videoEnabled : true))
                .map((group) => ({ ...group, items: filtered.filter((asset) => asset.kind === group.kind) }))
                .filter((group) => group.items.length > 0),
        [filtered, services.imageEnabled, services.videoEnabled],
    );
