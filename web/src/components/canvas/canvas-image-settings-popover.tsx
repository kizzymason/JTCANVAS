...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft" }: CanvasImageSettingsPopoverProps) {
    ...
    const maxCount = modelFeaturesOf(models.find((item) => item.value === normalizeModelOptionValue(config.model, models) && item.capability === "image")).maxCount;
...
function ImageSettingsPortal({
    ...
    return createPortal(
        ...
        >
            <ImageSettingsPanel config={config} onConfigChange={(key, value) => onConfigChange(key, value)} theme={theme} className="space-y-4" modelValue={config.model} />
        </div>,
        document.body,
    );
}
